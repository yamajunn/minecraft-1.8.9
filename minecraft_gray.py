...
for file in files:
    ...
    omission_list = ["wool", "water", "lava", "planks", "log", "sponge", "hardened", "glass", "end_stone", "emerald_block", "diamond_block", "bed", "ladder", "tnt"]

    if file != ".DS_Store" and ext == ".png" and not file in omission_list:
        ...
        for i in range(h):
            for j in range(w):
                b, g, r, a = img[i, j]
                img[i, j] = int((b+g+r)/3)+50, int((b+g+r)/3)+50, int((b+g+r)/3)+50, a
        # r_average = 0
        # g_average = 0
        # b_average = 0
